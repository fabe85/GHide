 ...
 * @since 2010-07-03
 */
public final class WaxmanGraphGenerator2<V extends IVertex, E extends IEdge>
        ...
        public WaxmanGraphGenerator2(double alpha, double beta, boolean bidirectional) {
        ...
        public WaxmanGraphGenerator2(double alpha, double beta, boolean bidirectional, long fixedSeed) {
        if (Double.isNaN(alpha) || Double.isInfinite(alpha)
                        || Double.isNaN(beta) || Double.isInfinite(beta))
                throw new IllegalArgumentException();
        if (!(0 < alpha && beta <= 1))
        ...
        public void generate(ILayer<V, E> g) {
                ...
                for (V v : g.getVertices())
                        for (V w : g.getVertices()) {
                                ...
                                final double p_waxman = Math.pow(alpha, 2) * Math.exp(-d / (beta * L));
